...
def score_sequence(model, alphabet, coords, seq, device):
    ...
    ll_fullseq = -avgloss.cpu().detach().numpy().item()
    ...
    ll_withcoord = -avgloss.cpu().detach().numpy().item()

    return ll_fullseq, ll_withcoord


def get_encoder_output(model, alphabet, coords, device):
    batch_converter = CoordBatchConverter(alphabet)
